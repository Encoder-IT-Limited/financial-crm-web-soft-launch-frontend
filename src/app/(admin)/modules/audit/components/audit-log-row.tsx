    ...
    <div className="border-b border-border last:border-b-0">
      ...
      >
        ...
        <span className="text-[12.5px] font-semibold text-text">
          {entry.entity} · {entry.entityLabel}
        </span>
        {!hideTenant && entry.tenantName && (
          <span className="text-[11.5px] text-text-3">{entry.tenantName}</span>
        )}
        <span className="text-[11px] text-text-4">{entry.module}</span>
        <span className="ml-auto text-[11px] text-text-4">{fmtDateTime(entry.timestamp)}</span>
        <span className="text-[11px] text-text-4">{entry.userName}</span>
        {hasDiff && (
