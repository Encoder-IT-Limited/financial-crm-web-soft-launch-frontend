/** Read-only demo catalog for POS's product search/register screen.
 *
 * Deliberately NOT imported from Inventory — Inventory has no real API yet
 * (still local mock data, see docs/missing-inventory.md) and Sales &
 * Invoicing's line items currently have no product-linking at all either
 * (see docs/missing-sales-invoice.md §0). This keeps POS self-contained
 * with its own per-warehouse stock breakdown until a real Inventory
 * Products API exists for every module to share.
 *
 * Swap for a real `GET /products?warehouseId=` call once that exists —
 * nothing downstream of `productLookupApi` needs to change shape for
 * that swap. */
