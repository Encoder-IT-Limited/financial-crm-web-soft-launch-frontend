...
export function SalesDashboard() {
  ...
  if (isLoading) {
    return <div className="h-64 animate-pulse rounded-[10px] border border-border bg-surface-subtle" />;
  }
    ...
    <div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <Card className="gap-3 p-5">
          ...
          <div className="flex h-36 items-end gap-3">
            {trend.map((bucket) => (
              <div key={bucket.key} className="flex h-full flex-1 flex-col items-center justify-end gap-1.5">
                ...
                <div className="flex min-h-4 w-full flex-1 items-end rounded-md bg-surface-subtle">
                  <div
                    className="w-full rounded-md bg-blue transition-all"
                    style={{ height: `${Math.max(4, (bucket.total / trendMax) * 100)}%` }}
                  />
                </div>
        ...
        <Card className="gap-3 p-5">
          ...
          <div className="flex flex-col gap-2.5">
            {statusBreakdown.rows.length === 0 && (
              <div className="py-6 text-center text-[12.5px] text-text-4">No invoices yet.</div>
            )}
            {statusBreakdown.rows.map(({ status, count }) => (
              <div key={status}>
                ...
                <div className="h-1.5 overflow-hidden rounded-full bg-surface-subtle">
                  <div
                    className={cn("h-full rounded-full", STATUS_TONE_BAR[STATUS_CONFIG[status].tone])}
                    style={{ width: `${(count / statusBreakdown.max) * 100}%` }}
                  />
                </div>
              </div>
            ))}
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <Card className="gap-0 p-0">
          ...
          {invoices.length === 0 ? (
            <div className="px-5 py-6 text-center text-[12.5px] text-text-4">No invoices yet.</div>
          ) : (
        ...
        <Card className="gap-0 p-0">
          <div className="border-b border-border px-5 py-3 text-sm font-bold text-text">Recent payments</div>
...
const STATUS_TONE_BAR: Record<"green" | "amber" | "red" | "neutral" | "blue", string> = {
  green: "bg-green",
  amber: "bg-amber",
  red: "bg-red",
  blue: "bg-blue",
